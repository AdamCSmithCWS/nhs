...
model {
 ...
 for(c in 1:ncastes){
  ...
  for(c in 1:ncastes){
    ## harvest rate priors onhunter variance, vary by year
    mulogtauhunter[c] ~ dnorm(0.0,0.01) #log-scale first-year precision
    taulogtauhunter[c] ~ dgamma(2,0.2) #random walk variance
 ...
 tauyear_day ~ dgamma(0.001,0.001)
  ...
  for(c in 1:ncastes){
    for(y in 1:nyears){
      ...
      mean_totkill_yc[y,c] <- exp(ann[y] + cst[c,y] + ann_day[y] + cst_day[c,y] + retrans_hunter_day[c] + retrans_hunter[c,y])
