...
model {
  ...
  for(i in 1:nhs){
   ...
   ### successful hunters, alternative zero inflated approach
    ...
    elambda1[i] <- ann + cst[caste[i],year[i]] + hntr[caste[i],year[i],hunter[i]] + elambda_day[i] #elambda_day[i] acts as an offset on effort
    ...
    elambda_day[i] <- ann_day + cst_day[caste[i],year[i]] + hntr_day[caste[i],year[i],hunter[i]]
  ...
  cst_day[1,1] <- 1
  for(c in 2:ncastes){
  cst[c,1] ~ dnorm(0,0.001)
  cst_day[c,1] ~ dnorm(0,0.001)
  }
  for(y in 2:nyears){  ### time series model for caste effects, accounting for all of the annual effects
    for(c in 1:ncastes){
  ...
  ann ~ dnorm(0,0.001) # fixed effects for year-1 annual harvest level
  ann_day ~ dnorm(0,0.001) # fixed effect for year-1 annual activity level
  ...
  for(c in 1:ncastes){
    for(y in 1:nyears){
      ...
      # estimate of the mean (per hunter) kill per caste, and year
        ...
        totkill_hcy[y,c,h] <- exp(ann + cst[c,y] + hntr[c,y,h] + ann_day + cst_day[c,y] + hntr_day[c,y,h])
        totdays_hcy[y,c,h] <- exp(ann_day + cst_day[c,y] + hntr_day[c,y,h])
        }
      ...
      mean_totkill_yc[y,c] <- exp(ann + cst[c,y] + ann_day + cst_day[c,y] + retrans_hunter_day[c] + retrans_hunter[c])
      mean_totdays_yc[y,c] <- exp(ann_day + cst_day[c,y] + retrans_hunter_day[c])
