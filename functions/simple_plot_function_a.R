...
plot_simple_a<- function(dat = both_a,
                        startYear = 1976){
...
npg = floor(nrow(unique(dat[,c("province","species")]))/9)
  rem = nrow(unique(dat[,c("province","species")]))-(npg*9)
  if(rem > 1){
  ...
  outgg = vector("list",length = npg)
      ...
      for(i in 1:npg){
        ...
        facet_wrap_paginate(~province+species,ncol = 3,nrow = 3,scales = "free",page = i)
  ...
  if(rem > 1){
    ...
    outgg[[npg+1]] = ggplot(data = dat,aes(x = year,y = mean,fill = mod))+
      ...
      theme(legend.position = "right",
            title = element_text(size = 9))+
      coord_cartesian(xlim = c(startYear, 2019))+
      facet_wrap_paginate(~province+species,ncol = nc,nrow = nr,scales = "free",page = i)
  }
