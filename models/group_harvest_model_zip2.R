...
#logistic random-walk time series model for psi
 ...
 tauyear_day ~ dgamma(0.001,0.001)
  ...
  ### 1 - proportional distribution of  harvest across groups through time years
 ###same basic model as the proportional distribution across periods for the geese, ducks, and muures

#####################################################################
  ### this component ignores caste specific variation in the group harvest actvities
  ### multinomial distribution across periods  
  #### nhunt_group[group,year]
  #### nhunter_other[year]
  ### kill_pyh and nkill_yh[y,h] are data
  for (y in 1:nyears) {

      act_other[1:ngroups,y] ~ dmulti(pkill_py[1:nperiods,y], nkill_yh[y,h])   # multinom distr vector responses
